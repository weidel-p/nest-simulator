...
#ifndef VOLUME_TRANSMITTER2_H
#define VOLUME_TRANSMITTER2_H
...
Name: volume_transmitter2 - Node used in combination with neuromodulated synaptic
plasticity. It collects all spikes emitted by the population of neurons
connected to the volume transmitter2 and transmits the signal to a user-specific
subset of synapses.

Description:
The volume transmitter2 is used in combination with neuromodulated
...
connected to the volume transmitter2 and delivers the spikes to a
...
transmitter2.  The neuromodulatory dynamics is calculated in the
synapses itself. The volume transmitter2 interacts in a hybrid
...
transmitter2, the volume transmitter2 is passed as a parameter when a
neuromodulatory synapse is defined. The implementation is based on the
framework presented in [1].

Examples:
/volume_transmitter2 Create /vol Set
...
deliver_interval - time interval given in d_min time steps, in which
                   ...
                   transmitter2 to the assigned synapses
 ...
 * synapses. In addition the synapses can ask the volume transmitter2 to deliver
 * the elements stored in the update() method with the method deliver_spikes().
 *
 * @ingroup Devices
 */
class volume_transmitter2 : public Archiving_Node
{

public:
  volume_transmitter2();
  volume_transmitter2( const volume_transmitter2& );
...
private:
  ...
  struct Buffers_
  {
    ...
    double n_;
    double last_update_;
    std::map<double, double> history_;
  };
...
inline port
volume_transmitter2::handles_test_event( SpikeEvent&, rport receptor_type )
{
  //register_stdp_connection( kernel().simulation_manager.get_time().get_ms() );
  if ( receptor_type != 0 )
...
inline void
volume_transmitter2::get_status( DictionaryDatum& d ) const  
{
  
  def< double >( d, "n", B_.n_);
...
inline void
volume_transmitter2::set_status( const DictionaryDatum& d )
{
...
#endif /* #ifndef VOLUME_TRANSMITTER2_H */
