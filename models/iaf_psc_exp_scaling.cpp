...
nest::RecordablesMap< nest::iaf_psc_exp_scaling > nest::iaf_psc_exp_scaling::recordablesMap_;
...
template <>
void
RecordablesMap< iaf_psc_exp_scaling >::create()
{
  // use standard names whereever you can for consistency!
  insert_( names::V_m, &iaf_psc_exp_scaling::get_V_m_ );
  insert_( names::weighted_spikes_ex, &iaf_psc_exp_scaling::get_weighted_spikes_ex_ );
  insert_( names::weighted_spikes_in, &iaf_psc_exp_scaling::get_weighted_spikes_in_ );
  insert_( names::I_syn_ex, &iaf_psc_exp_scaling::get_I_syn_ex_ );
  insert_( names::I_syn_in, &iaf_psc_exp_scaling::get_I_syn_in_ );
}
}
...
nest::iaf_psc_exp_scaling::Parameters_::Parameters_()
  : Tau_( 10.0 )             // in ms
  , C_( 250.0 )              // in pF
  , t_ref_( 2.0 )            // in ms
  , E_L_( -70.0 )            // in mV
...
nest::iaf_psc_exp_scaling::State_::State_()
  : i_0_( 0.0 )
  , i_syn_ex_( 0.0 )
  , i_syn_in_( 0.0 )
  , V_m_( 0.0 )
...
void
nest::iaf_psc_exp_scaling::Parameters_::get( DictionaryDatum& d ) const
{
...
double
nest::iaf_psc_exp_scaling::Parameters_::set( const DictionaryDatum& d )
{
...
void
nest::iaf_psc_exp_scaling::State_::get( DictionaryDatum& d, const Parameters_& p ) const
{
  def< double >( d, names::V_m, V_m_ + p.E_L_ ); // Membrane potential
}

void
nest::iaf_psc_exp_scaling::State_::set( const DictionaryDatum& d,
  const Parameters_& p,
  double delta_EL )
{
...
nest::iaf_psc_exp_scaling::Buffers_::Buffers_( iaf_psc_exp_scaling& n )
  : logger_( n )
{
}

nest::iaf_psc_exp_scaling::Buffers_::Buffers_( const Buffers_&, iaf_psc_exp_scaling& n )
  : logger_( n )
{
}
...
nest::iaf_psc_exp_scaling::iaf_psc_exp_scaling()
  : Archiving_Node()
  , P_()
  , S_()
  , B_( *this )
...
nest::iaf_psc_exp_scaling::iaf_psc_exp_scaling( const iaf_psc_exp_scaling& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::iaf_psc_exp_scaling::init_state_( const Node& proto )
{
  const iaf_psc_exp_scaling& pr = downcast< iaf_psc_exp_scaling >( proto );
  S_ = pr.S_;
}

void
nest::iaf_psc_exp_scaling::init_buffers_()
{
  B_.spikes_ex_.clear(); // includes resize
  B_.spikes_in_.clear(); // includes resize
  B_.currents_.clear();  // includes resize
  B_.inc_conns_.clear();  // includes resize
  B_.logger_.reset();
  Archiving_Node::clear_history();
}

void
nest::iaf_psc_exp_scaling::calibrate()
{
  ...
  // a double in ms. The grid based iaf_psc_exp_scaling can only handle refractory
...
void
nest::iaf_psc_exp_scaling::update( const Time& origin, const long from, const long to )
{
  assert(
...
void
nest::iaf_psc_exp_scaling::handle( SpikeEvent& e )
{
...
void
nest::iaf_psc_exp_scaling::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
nest::iaf_psc_exp_scaling::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
