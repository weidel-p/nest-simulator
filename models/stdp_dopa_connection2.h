...
#ifndef STDP_DOPA_CONNECTION2_H
#define STDP_DOPA_CONNECTION2_H
 ...
 * connection.
 */
class STDPDopa2CommonProperties : public CommonSynapseProperties
{
public:
  /**
   * Default constructor.
...
inline long
STDPDopa2CommonProperties::get_vt_gid() const
{
  if ( vt_ != 0 )
    return vt_->get_gid();
  else
...
/**
 * Class representing an STDPDopaConnection2 with homogeneous parameters,
 * i.e. parameters are the same for all synapses.
 */
template < typename targetidentifierT >
class STDPDopaConnection2 : public Connection< targetidentifierT >
{

public:
  typedef STDPDopa2CommonProperties CommonPropertiesType;
   ...
   */
  STDPDopaConnection2( const STDPDopaConnection2& );
   ...
   */
  void send( Event& e, thread t, double, const STDPDopa2CommonProperties& cp );
...
private:
   void update_weight_( double c0,
    double n0,
    double minus_dt,
    const STDPDopa2CommonProperties& cp );
  ...
  void facilitate_( double kplus, const STDPDopa2CommonProperties& cp );
  void depress_( double kminus, const STDPDopa2CommonProperties& cp );
...
//
// Implementation of class STDPDopaConnection2.
//

template < typename targetidentifierT >
STDPDopaConnection2< targetidentifierT >::STDPDopaConnection2()
  : ConnectionBase()
  , weight_( 1.0 )
  , Kplus_( 0.0 )
  , c_( 0.0 )
...
template < typename targetidentifierT >
void
STDPDopaConnection2< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
...
template < typename targetidentifierT >
void
STDPDopaConnection2< targetidentifierT >::set_status( const DictionaryDatum& d,
  ConnectorModel& cm )
{
...
template < typename targetidentifierT >
inline void
STDPDopaConnection2< targetidentifierT >::update_weight_( double c0,
  double n0,
  double minus_dt,
  const STDPDopa2CommonProperties& cp )
{
  const double taus_ = ( cp.tau_c_ + cp.vt_->get_tau_n() ) / ( cp.tau_c_ * cp.vt_->get_tau_n() );
  weight_ = weight_
...
template < typename targetidentifierT >
inline void
STDPDopaConnection2< targetidentifierT >::process_dopa_spikes_(
...
template < typename targetidentifierT >
inline void
STDPDopaConnection2< targetidentifierT >::facilitate_( double kplus,
  const STDPDopa2CommonProperties& cp )
{
  c_ += cp.A_plus_ * kplus;
}
...
template < typename targetidentifierT >
inline void
STDPDopaConnection2< targetidentifierT >::depress_( double kminus,
  const STDPDopa2CommonProperties& cp )
{
...
template < typename targetidentifierT >
inline void
STDPDopaConnection2< targetidentifierT >::send( Event& e,
  thread t,
  double,
  const STDPDopa2CommonProperties& cp )
{
  ...
  //std::cout << cp.vt_->get_n(e.get_stamp().get_ms()) << " " << std::endl;
  ...
  cp.vt_->get_history( t_last_update_ - dendritic_delay,
...
//  while ( start != finish )
//  {
//    double t = start->t_ + dendritic_delay;
...
#endif // of #ifndef STDP_DOPA_CONNECTION2_H
