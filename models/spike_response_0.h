...
#ifndef SPIKE_RESPONSE_0_H
#define SPIKE_RESPONSE_0_H
...
{
/* BeginDocumentation
   Name: spike_response_0 - Leaky integrate-and-fire neuron model with exponential
                       PSCs.

   Description:
   spike_response_0p is an implementation of a leaky integrate-and-fire model
   ...
   optimization levels. A future version of spike_response_0 will probably
...
Remarks:
   ...
   spike_response_0 can handle current input in two ways: Current input
 ...
 */
class spike_response_0 : public Archiving_Node
{

public:
  spike_response_0();
  spike_response_0( const spike_response_0& );
...
private:
  ...
  friend class RecordablesMap< spike_response_0 >;
  friend class UniversalDataLogger< spike_response_0 >;
  ...
  struct Parameters_
  {
    ...
    // scaling factor for refacoriness
    double xi_;
  ...
  struct State_
  {
    ...
    double t_last_spike_;      //!< time of last spike 
  ...
  struct Buffers_
  {
    Buffers_( spike_response_0& );
    Buffers_( const Buffers_&, spike_response_0& );
    ...
    UniversalDataLogger< spike_response_0 > logger_;
  };
  ...
  /**
   * @defgroup spike_response_0_data
  ...
  static RecordablesMap< spike_response_0 > recordablesMap_;
};


inline port
nest::spike_response_0::send_test_event( Node& target,
  rport receptor_type,
  synindex,
  bool )
{
...
inline port
spike_response_0::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
spike_response_0::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
spike_response_0::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d, P_ );
...
inline void
spike_response_0::set_status( const DictionaryDatum& d )
{
