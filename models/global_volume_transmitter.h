...
#ifndef GLOBAL_VOLUME_TRANSMITTER_H
#define GLOBAL_VOLUME_TRANSMITTER_H
...
Name: global_volume_transmitter - Node used in combination with neuromodulated synaptic
plasticity. It collects all spikes emitted by the population of neurons
connected to the global_volume transmitter and transmits the signal to a user-specific
subset of synapses.

Description:
The global_volume transmitter is used in combination with neuromodulated
...
connected to the global_volume transmitter and delivers the spikes to a
...
emitted by the population of neurons connected to the global_volume
transmitter.  The neuromodulatory dynamics is calculated in the
synapses itself. The global_volume transmitter interacts in a hybrid
...
insure the link between the neuromodulatory synapses and the global_volume
transmitter, the global_volume transmitter is passed as a parameter when a
neuromodulatory synapse is defined. The implementation is based on the
framework presented in [1].

Examples:
/global_volume_transmitter Create /vol Set
...
deliver_interval - time interval given in d_min time steps, in which
                  the global_volume signal is delivered from the global_volume
...
/**
* global_volume transmitter class.
*
...
* synapses. In addition the synapses can ask the global_volume transmitter to deliver
* the elements stored in the update() method with the method deliver_spikes().
*
* @ingroup Devices
*/
class global_volume_transmitter : public Archiving_Node
{

public:
 global_volume_transmitter();
 global_volume_transmitter( const global_volume_transmitter& );
...
inline port
global_volume_transmitter::handles_test_event( SpikeEvent&, rport receptor_type )
{
 if ( receptor_type != 0 )
 {
...
inline void
global_volume_transmitter::get_status( DictionaryDatum& d ) const
{
 P_.get( d );
...
inline void
global_volume_transmitter::set_status( const DictionaryDatum& d )
{
...
#endif /* #ifndef GLOBAL_VOLUME_TRANSMITTER_H */
