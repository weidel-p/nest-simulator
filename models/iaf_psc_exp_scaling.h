...
#ifndef IAF_PSC_EXP_SCALING_H
#define IAF_PSC_EXP_SCALING_H
...
{
/* BeginDocumentation
   Name: iaf_psc_exp_scaling - Leaky integrate-and-fire neuron model with exponential
                       PSCs.

   Description:
   iaf_psc_exp_scalingp is an implementation of a leaky integrate-and-fire model
   ...
   optimization levels. A future version of iaf_psc_exp_scaling will probably
...
Remarks:
   ...
   iaf_psc_exp_scaling can handle current input in two ways: Current input
   ...
   SeeAlso: iaf_psc_exp_scaling_ps
 ...
 */
class iaf_psc_exp_scaling : public Archiving_Node
{

public:
  iaf_psc_exp_scaling();
  iaf_psc_exp_scaling( const iaf_psc_exp_scaling& );
...
private:
  ...
  friend class RecordablesMap< iaf_psc_exp_scaling >;
  friend class UniversalDataLogger< iaf_psc_exp_scaling >;
  ...
  struct Buffers_
  {
    Buffers_( iaf_psc_exp_scaling& );
    Buffers_( const Buffers_&, iaf_psc_exp_scaling& );

    /** buffers and sums up incoming spikes/currents */
    RingBuffer spikes_ex_;
    RingBuffer spikes_in_;
    std::vector< RingBuffer > currents_;
    std::vector< double > inc_conns_;

    //! Logger for all analog data
    UniversalDataLogger< iaf_psc_exp_scaling > logger_;
  };
  ...
  /**
   * @defgroup iaf_psc_exp_scaling_data
  ...
  static RecordablesMap< iaf_psc_exp_scaling > recordablesMap_;
};


inline port
nest::iaf_psc_exp_scaling::send_test_event( Node& target,
  rport receptor_type,
  synindex,
  bool )
{
...
inline port
iaf_psc_exp_scaling::handles_test_event( SpikeEvent& e, rport receptor_type )
{
  std::cout << "handle test ev. sender "<< e.get_weight() << std::endl;
  if ( receptor_type != 0 )
  {
...
inline port
iaf_psc_exp_scaling::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type == 0 )
  {
    return 0;
...
inline port
iaf_psc_exp_scaling::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
iaf_psc_exp_scaling::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d, P_ );
...
inline void
iaf_psc_exp_scaling::set_status( const DictionaryDatum& d )
{
